...
$categories = $conn->query("SELECT * FROM categories", 1)->fetch_array();
if (!$categories) {
    die('<script>location.href="/home"</script>');
}
$get = $conn->query("SELECT * FROM webinars WHERE type = 'text_lesson' ORDER BY id DESC");
$count = $get->num_rows;
?>
            ...
            <div id="topFilters" class="shadow-lg border border-gray300 rounded-sm p-10 p-md-20">
                <div class="row align-items-center">
                    <div class="col-lg-6 d-flex align-items-center">
                        <select id="sort" class="form-control font-14" onchange="fitler_course()" <?php if ($count == 0) {
                                                                                                        echo 'disabled';
                                                                                                    } ?>>
                    ...
                    <div class="col-lg-6 d-flex align-items-center">
                        <select id="price" class="form-control font-14" onchange="fitler_course()" <?php if ($count == 0) {
                                                                                                        echo 'disabled';
                                                                                                    } ?>>
                ...
                <div class="col-12 col-lg-4">
                    <div class="mt-20 p-20 rounded-sm shadow-lg border border-gray300 filters-container">
                        <div class="input-group">
                            <input type="search" id="char" onchange="fitler_course()" autocomplete="off" class="form-control rounded" placeholder="Tìm kiếm từ khóa..." <?php if ($count == 0) {
                                                                                                                                                                            echo 'disabled';
                                                                                                                                                                        } ?> />
                        </div>

                        <button type="reset" onclick="clear_fitler()" class="btn btn-sm btn-primary btn-block mt-30" <?php if ($count == 0) {
                                                                                                                            echo 'disabled';
                                                                                                                        } ?>>Xóa lọc</button>
                    </div>
                </div>
...
<script>
    ...
    function load_course() {
        $("#loading").show();
        $(".list_course").hide();
        $.post("<?= $domain; ?>model/categories/data.php?query=all", {
                page: page,
                char: char,
                price: price,
                sort: sort,
