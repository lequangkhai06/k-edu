...
if ($quiz["password"] != null) {
    if (isset($quiz[$session_namespace]["password"])) {
        if ($quiz[$session_namespace]["password"] != $quiz["password"]) {
            unset($quiz[$session_namespace]["password"]);
            die('<script>location.href="/quizzes/' . $id . '/prepare"</script>');
        }
    }
}
...
// lấy số lần tham gia
$query_quiz_result = $conn->query("SELECT * FROM quizzes_results WHERE quiz_id = '{$id}' and user_id = '{$user_id}'");
$total_quiz_result = $query_quiz_result->num_rows;


if ($total_quiz_question == 0) {
    echo '<script>
    Toastify({
        text: "CHƯA CÓ CÂU HỎI NÀO! VUI LÒNG QUAY LẠI SAU!",
        duration: 3000,
        newWindow: true,
        close: true,
        gravity: "bottom",
...
if ($total_quiz_result >= $quiz["attempt"]) {
    echo '<script>
    Toastify({
...
<div class="container mb-4">
            ...
            <div class="row">
                <div class="col-6 col-md-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <strong class="font-30 font-weight-bold text-secondary mt-5">??/<?= $quiz["total_mark"]; ?></strong>
                ...
                <div class="col-6 col-md-3 d-flex align-items-center justify-content-center">
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <strong class="font-30 font-weight-bold text-secondary mt-5"><?= $total_quiz_result; ?>/<?= $quiz["attempt"]; ?></strong>
    ...
    <section class="mt-30 quiz-form">
        <form action="#" method="post" id="quiz_form">
            ...
            <input type="hidden" name="session_name" value="<?= $session_namespace; ?>" class="form-control" placeholder="" />
            <div class="activities-container shadow-sm rounded-lg mt-25 p-20 p-lg-35">
                <?php
                while ($question = $quiz_question->fetch_array()) {
                    $quizzes_questions_answers = $conn->query("select * from quizzes_questions_answers where question_id = '{$question["id"]}' order by rand()");
                ?>
                    <!-- qs -->
                    <div class="question">
                            ...
                            ?>
                                <div class="list-group-item list-group-item-action" aria-current="true">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" value="<?= $answers["id"]; ?>" type="radio" name="<?= $answers["question_id"]; ?>" id="<?= $answers["question_id"] + $i; ?>">
...
if (!isset($_SESSION[$session_namespace]['end_time'])) {
    ...
    $_SESSION[$session_namespace]['end_time'] = strtotime('+' . $quiz["time"] . ' minutes', time()) * 1000;
}
?>
<script>
    Toastify({
                    ...
                    Swal.fire(response.title, response.message, response.type).then(function() {
                        if (response.reload) {
                            if (response.type == 'success') {
                                $.ajax({
                                    type: "POST",
                                    url: "<?= $domain; ?>handler/execute/quiz.php?action=getid&quiz_id=<?= $id; ?>",
                                    dataType: "json",
                                    success: function(data) {
                                        if (data.status == "success") {
                                            window.location.href = "/quizzes/" + data.quiz_id + "/" + data.latest_id + "/result";
                                        } else {
