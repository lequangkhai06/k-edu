...
<?php if ($_SERVER['REQUEST_METHOD'] == 'POST') { ?>
    ...
    $query_category = "AND category_id = '{$categories['id']}'";
    if ($query == "all") {
        $query_category = "";
    }
    $total_records = $conn->query("SELECT * FROM webinars WHERE type = 'text_lesson' $sql_price $query_category $sql_char")->num_rows;
    $config = array(
        "current_page" => $page,
    ...
    $paging->init($config);
    $get = $conn->query("SELECT * FROM webinars WHERE type = 'text_lesson' $sql_price $query_category $sql_char ORDER BY $sql_sapxep LIMIT {$paging->getConfig()['start']}, {$paging->getConfig()['limit']}");
    if ($get->num_rows > 0) {
...
<?php } else {  ?>
    <p class="text-danger">Phương thức không được phép.</p>
    <!-- js -->
